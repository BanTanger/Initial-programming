...
void direction(Point2f& cc,Point2f& precenter,Point2f& center) {
	...
	/* 这里是因为我用的是两帧向量位置来判断方向，而视频中有一瞬间本来顺时针走的能量装甲，被击中后未激活的锤子从后面出现了。 */
	bool flag = false;
	if (fangcheng > 0) {
		putText(srcImage, "COUNTERCLOCKWISE", cc, FONT_HERSHEY_SIMPLEX, 0.8, Scalar(127,127,127),1.5);//打印字体
		flag = true;
	}
	else putText(srcImage, "CLOCKWISE", cc, FONT_HERSHEY_SIMPLEX, 0.8, Scalar(127, 127, 127), 1.5);//打印字体
	if(count < 2) count++;
	if (count > 1 && flag == false) putText(srcImage, "COUNTERCLOCKWISE", cc, FONT_HERSHEY_SIMPLEX, 0.8, Scalar(0, 0, 127), 1.5);//打印字体
	if (count > 1 && flag == true) putText(srcImage, "CLOCKWISE", cc, FONT_HERSHEY_SIMPLEX, 0.8, Scalar(0, 0, 127), 1.5);//打印字体
}    
int hist_w = 512;
int hist_h = 400;
int width = 2;
...
void Spot::findcontoursfun(Mat& drawcircle) {
	...
	for (int j = 0; j < contours.size(); j++) {
		...
		if ((area_of_contour >= 1200) && (area_of_contour <= 2000)) {//只有一个子轮廓的情况
			...
			prebox = minAreaRect(contours[hierarchy[j][2]]);//把轮廓的所有点存入旋转矩形中。
			...
			Point2f precenter = minAreaRect(contours[hierarchy[(j + 1)][2]]).center;//得到上一个旋转矩形的中心点。
			...
			rescenterPoint = Point2f(meanPoints.x + cosA * xx - sinA * yy, meanPoints.y + sinA * xx + cosA * yy);//可以作为偏移量。
...
#endif // SHOW_CIRCLE
			/*因为窗口抖动，我的圆心是拟合圆的圆心，所以会产生一定误差，暂时没有办法和技术来解决。*/
			for (int i = 0; i < 4; i++) {
				cout << "第" << i << "个坐标点" << points[i] << endl;
				line(srcImage, re[i], re[(i + 1) % 4], Scalar(0, 127, 127), 2, CV_AA);//预测框。
				//line(srcImage, points[i], points[(i + 1) % 4], Scalar(127, 127, 0), 2, CV_AA);//原始框
			}
			Scalar color(rand() & 255, rand() & 255, rand() & 255);
...
/*
		...
		又一个历时两天的bug被解决，预测方框如何做偏移量，我一开始直接遍历点去相加预测点，
		但第一，忘记减去方框中心点，导致坐标特别大而且不是想要的，第二，操作有误：points[i] += respoints,这样points的坐标越来越大，而不是想要的四个点平移的结果。
		通过创建一个新的变量来放新的数据，终于算出。

		方向预测，这里用到一些数学知识，两个向量做叉乘，得到的如果是负数就是顺时针转动，反之。
*/
static bool CircleInfo2(vector<Point2f>& m_Points, Point2f& Centroid, float& dRadius)
{
	if (!m_Points.empty())
