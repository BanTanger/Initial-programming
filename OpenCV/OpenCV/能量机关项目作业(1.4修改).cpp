...
void Pretreat::prethreshold() {
	...
	// 2.二值化，背景黑，图案白
	threshold(midImage, midImage,30,255, THRESH_BINARY|THRESH_OTSU);
#ifdef DEBUG
	//读取二值化视频
...
void Spot::findcontoursfun() {
	...
	*/
#ifdef DEBUG
		cout << contour[hierarchy[i][3]] << endl;
	...
	for (int j = 0; j < contours.size(); j++) {
		...
		if ( (area_of_contour >= 1200) && (area_of_contour <= 2000)) {//只有一个子轮廓的情况
			...
			center = (points[0] + points[2]) / 2;//返回中心坐标
			Scalar color(rand() & 255, rand() & 255, rand() & 255);
			putText(srcImage, "target", points[0], FONT_HERSHEY_SIMPLEX, 1.0, color,1.5);//打印字体
...
class DrawCircle {
public:
	void CircleFunc(Mat& drawcircle);
};
void DrawCircle::CircleFunc(Mat& drawcircle) {
	Point2f cc = Point2f(0, 0);
	vector<Point2f> cirV;
	Point center = box.center;//得到旋转矩形的中心点。
	circle(srcImage, center, 1, Scalar(0, 255, 0), 2);//打印出中心点。
	circle(drawcircle, center, 1, Scalar(0, 0, 255), 1);
