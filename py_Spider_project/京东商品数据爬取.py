...
def drop_down():
    ...
    for x in range(1,12,2): # 1 3 5 7 9 11 控制高度变化
        time.sleep(1)
        j = x / 11 # 1/11,3/11,5/11,7/11,9/11,11/11
        # document.documentElement.scrllTop 指定滚动条的位置
        # document.documentElement.scrllHeight 滚动条最大高度
        js = 'document.documentElement.scrllTop = document.documentElement.scrllHeight * %f' % j
...
for li in lis:
    ...
    price = li.find_element_by_css_selector(' .p-price i').text # 商品价格
