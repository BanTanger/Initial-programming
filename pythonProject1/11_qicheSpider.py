...
class CarSpider:
    ...
    def get_html(self,url):
        ...
        html = res.read().decode('utf-8','ignore') #ignore 忽略特殊字符，解决反爬
    ...
    def get_data(self,car_url):
        ...
        two_regex = '<div class="car-box">.*?<h3 class="car-brand-name">(.*?)</h3>.*?<h4>(.*?)</h4>.*?<h4>(.*?)</h4>.*?<h4>(.*?)</h4>.*?<h4>(.*?)</h4>.*?<div class="goodstartmoney">(.*?)</div>.*?</div>'
        ...
        item['displace'] = car_list[0][4].split('/')[1].strip()
        item['city'] = car_list[0][5].strip()
        item['price'] = car_list[0][6].strip()
